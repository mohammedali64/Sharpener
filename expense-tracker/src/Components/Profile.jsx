...
const Profile = () => {
  ...
  return (
    <div>
      ...
      <div className='w-2xl mt-10 border-b-[1px] pb-5'>
        <form onSubmit={handleUpdate}>
        <div className='flex flex-row justify-between'>
            <div className='text-3xl font-bold'>Contact Details</div>
            <button className='cursor-pointer border-2 border-red-600 p-1 rounded-sm'>Cancel</button>
        </div>
        <div className='flex flex-row justify-between py-5'>
            <div>
                <label htmlFor="name">Full Name: </label>
                <input onChange={(e)=>setName(e.target.value)} id='name' name='name' className='border-2 rounded-md'/>
            </div>
            <div>
                <label htmlFor='photoUrl'>Profile Photo URL: </label>
                <input onChange={(e)=>setUrl(e.target.value)} id='photoUrl' name='photoUrl' className='border-2 rounded-md'/>
            </div>
        </div>
        <div>
            <div>
